#!/usr/bin/env python3
"""
Smart Firewall — AI Detection Component
Live network attack detector using ML + rule hybrid.

Usage: sudo python3 detector.py

Emits JSON events to ~/sfw/events.jsonl for the firewall agent to consume.
"""
...
packet_buffer = defaultdict(deque)   # src_ip -> deque of packet_info dicts
last_alert    = {}                   # (src_ip, class) -> timestamp
running       = True
...
def packet_handler(pkt):
    """AsyncSniffer callback — adds packet to per-source buffer."""
    if IP not in pkt:
        return
    src_ip = pkt[IP].src
...
def evict_old(buf, now, window_sec):
    """Remove packets older than the window from the buffer (in-place)."""
...
def compute_features(pkts):
    """Compute the 12 features from a list of packet info dicts."""
    if len(pkts) < 2:
        return None
...
def rule_based_detect(features):
    """Deterministic rules. Returns (label, conf, rule_name) or None."""
...
def emit_alert(src_ip, predicted_class, confidence, features, source):
    """Write an alert to the events file, respecting cooldown."""
...
def score_loop(model, feature_cols):
    """Main detection loop. Runs until SIGINT."""
    ...
    while running:
        ...
        for src_ip in list(packet_buffer.keys()):
            ...
            # Rule layer first (runs on small buffers)
            rule_result = rule_based_detect(features)
            if rule_result is not None:
                label, conf, rule_name = rule_result
                print(f"[{ts}] RULE  src={src_ip:15s} pred={label:18s} "
                      f"conf={conf:.2f} via={rule_name} pkts={features['pkt_count']} "
            ...
            # ML layer (only on larger buffers)
            ...
            print(f"[{ts}] {tag} src={src_ip:15s} pred={pred:18s} "
                  f"conf={conf:.3f} (thr={threshold:.2f}) pkts={features['pkt_count']}")
